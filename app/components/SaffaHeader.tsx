...
export function useSaffaLanguage(): [SaffaLanguage, (value: SaffaLanguage) => void] {
  ...
  useEffect(() => {
    ...
    const sync = (event: Event) => {
      const value = (event as CustomEvent<SaffaLanguage>).detail;
      if (value === 'ar' || value === 'en') setLanguageState(value);
    };
...
export default function SaffaHeader({ active = 'home', cartCount, cartAction }: { active?: 'home' | 'collection' | 'about' | 'contact'; cartCount?: number; cartAction?: () => void }) {
  ...
  return (
    <header className="saffa-site-header">
      ...
      <div className="saffa-site-actions">
        <div className="saffa-language" aria-label="Language selector">
          <button className={language === 'en' ? 'active' : ''} onClick={() => setLanguage('en')} type="button">EN</button>
          <span>/</span>
          <button className={language === 'ar' ? 'active' : ''} onClick={() => setLanguage('ar')} type="button">AR</button>
        </div>
        <button className="saffa-site-cart" type="button" onClick={cartAction} aria-label={`${t.cart}${cartCount !== undefined ? `, ${cartCount}` : ''}`}>
          <span>{t.cart}</span>
          {cartCount !== undefined && <b>{cartCount}</b>}
        </button>
      ...
      {mobileOpen && (
        <div className="saffa-mobile-menu">
          <a href="/" onClick={() => setMobileOpen(false)}>{t.home}</a>
          <a href="/grid" onClick={() => setMobileOpen(false)}>{t.collection}</a>
          <a href="/about" onClick={() => setMobileOpen(false)}>{t.about}</a>
          <a href="/contact" onClick={() => setMobileOpen(false)}>{t.contact}</a>
          <div className="saffa-mobile-language">
            <button className={language === 'en' ? 'active' : ''} onClick={() => setLanguage('en')} type="button">English</button>
            <button className={language === 'ar' ? 'active' : ''} onClick={() => setLanguage('ar')} type="button">العربية</button>
          </div>
        </div>
      )}
    </header>
