...
export default function Home() {
  ...
  const text = ar
    ? { scroll:'اسحب', open:'فتح', close:'إغلاق', add:'🛒 أضف إلى السلة', buy:'شراء الآن', selection:'اختياراتك', yourCart:'سلة التسوق', item:'منتج', items:'منتجات', empty:'سلتك فارغة', emptyText:'افتحي المنتج واضغطي على أضف إلى السلة لإضافته هنا.', continue:'متابعة التسوق', total:'الإجمالي', clear:'إفراغ السلة', checkout:'إتمام الطلب', buyCart:'شراء الآن', circle:'دائرة', product:'المنتج', sale:'خصم' }
    : { scroll:'Scroll', open:'Open', close:'Close', add:'🛒 Add to Cart', buy:'Buy Now', selection:'Your Selection', yourCart:'Your Cart', item:'item', items:'items', empty:'Your cart is empty.', emptyText:'Open a product and tap “Add to Cart” to add it here.', continue:'Continue Shopping', total:'Total', clear:'Clear Cart', checkout:'Checkout', buyCart:'Buy Now', circle:'Circle', product:'Product', sale:'SALE' };
  ...
  useEffect(() => { const timer = window.setTimeout(() => setView('product'), 2200); return () => window.clearTimeout(timer); }, []);
  ...
  function moveProduct(direction: 1 | -1) { setView('product'); setActiveIndex(current => { const next = current + direction; if (next < 0) return products.length - 1; if (next >= products.length) return 0; return next; }); }
  ...
  function orderOnWhatsApp() { if (!cart.length) return; const lines = cart.map(item => `• ${item.name}\n  Color: ${item.color || 'Not specified'}\n  Quantity: ${item.quantity}\n  Price: EGP ${item.price.toFixed(2)}`).join('\n\n'); const message = `Hello Saffa Fashion 👋\n\nI would like to place an order:\n\n${lines}\n\nTotal: EGP ${cartTotal.toFixed(2)}`; window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer'); }
  function buyNow(product: Product = activeProduct) { const message = `Hello Saffa Fashion 👋\n\nI would like to buy:\n\n${product.name}\nColor: ${product.color}\nPrice: EGP ${product.price.toFixed(2)}${product.originalPrice ? `\nOriginal price: EGP ${product.originalPrice.toFixed(2)}\nOffer: ${product.saleLabel || 'SALE'}` : ''}`; window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer'); }
  const circleProducts = useMemo(() => products.map((product, index) => ({ product, index, angle: -90 + index * (360 / products.length) })), []);
  ...
  return <main className="saffa-home" id="top">
    ...
    <section className={`saffa-product-stage saffa-${view}-view`} onWheel={handleWheel} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} aria-label={ar ? 'عرض منتجات صفا فاشن' : 'Saffa Fashion product showcase'}>
      <div className={`saffa-circle-intro ${view === 'circle' ? 'is-visible' : 'is-hidden'}`}>
        <div className="saffa-circle-heading"><span>SAFFA FASHION · {ar ? 'صفا فاشن' : 'COLLECTION'}</span><h1>{ar ? 'مجموعة صفا' : 'The Saffa Collection'}</h1></div>
        <div className="saffa-view-switch"><button className="selected" type="button">{text.circle}</button><button type="button" onClick={() => setView('product')}>{text.product}</button></div>
        <div className="saffa-clock" aria-label="Saffa collection selector"><div className="saffa-clock-ring" /><div className="saffa-clock-center"><span>SAFFA</span><strong>{String(products.length).padStart(2, '0')}</strong><small>{ar ? 'قطعة' : 'PIECES'}</small></div>{circleProducts.map(({ product, index, angle }) => <button key={product.slug} className={`saffa-clock-item ${index === activeIndex ? 'active' : ''}`} style={{ '--angle': `${angle}deg` } as React.CSSProperties} onClick={() => enterProduct(index)} aria-label={`${text.open} ${product.name} ${product.color}`}><span><img src={product.image} alt="" /></span><small>{String(index + 1).padStart(2, '0')}</small></button>)}</div>
        <div className="saffa-circle-scroll">{text.scroll}<i>↓</i></div>
      </div>
      <div className={`saffa-product-zoom ${view === 'product' ? 'is-visible' : 'is-hidden'}`}>
        <div className="saffa-product-track" style={{ transform: `translate3d(0, -${activeIndex * 100}%, 0)` }}>{products.map((product, index) => <article className="saffa-product-slide" key={product.slug} aria-hidden={index !== activeIndex}><button className="saffa-product-visual" onClick={() => setDetailOpen(true)} aria-label={`${text.open} ${product.name}`}><img src={product.image} alt={product.name} draggable={false} /></button></article>)}</div>
