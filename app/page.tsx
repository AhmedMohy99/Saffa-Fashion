...
export default function Home() {
  ...
  const text = ar
    ? { scroll: 'اسحب', open: 'فتح', close: 'إغلاق', add: '🛒 أضف إلى السلة', buy: 'شراء الآن', selection: 'اختياراتك', yourCart: 'سلة التسوق', item: 'منتج', items: 'منتجات', empty: 'سلتك فارغة', emptyText: 'افتحي المنتج واضغطي على أضف إلى السلة لإضافته هنا.', continue: 'متابعة التسوق', total: 'الإجمالي', clear: 'إفراغ السلة', checkout: 'إتمام الطلب', buyCart: 'شراء الآن', circle: 'دائرة', product: 'المنتج' }
    : { scroll: 'Scroll', open: 'Open', close: 'Close', add: '🛒 Add to Cart', buy: 'Buy Now', selection: 'Your Selection', yourCart: 'Your Cart', item: 'item', items: 'items', empty: 'Your cart is empty.', emptyText: 'Open a product and tap “Add to Cart” to add it here.', continue: 'Continue Shopping', total: 'Total', clear: 'Clear Cart', checkout: 'Checkout', buyCart: 'Buy Now', circle: 'Circle', product: 'Product' };
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setView('product'), 2200);
  ...
  function moveProduct(direction: 1 | -1) {
    setView('product');
    setActiveIndex(current => { const next = current + direction; if (next < 0) return products.length - 1; if (next >= products.length) return 0; return next; });
  }
  ...
  function buyNow(product: Product = activeProduct) {
    const message = `Hello Saffa Fashion 👋\n\nI would like to buy:\n\n${product.name}\nColor: ${product.color}\nPrice: EGP ${PRICE.toFixed(2)}`;
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer');
  }
  function buyCartNow() { orderOnWhatsApp(); }
    ...
    <main className="saffa-home" id="top">
      ...
      <section className={`saffa-product-stage saffa-${view}-view`} onWheel={handleWheel} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} aria-label={ar ? 'عرض منتجات صفا فاشن' : 'Saffa Fashion product showcase'}>
        <div className={`saffa-circle-intro ${view === 'circle' ? 'is-visible' : 'is-hidden'}`}>
          ...
          <div className="saffa-clock" aria-label="Saffa collection selector">
            <div className="saffa-clock-ring" />
            <div className="saffa-clock-center"><span>SAFFA</span><strong>{String(products.length).padStart(2, '0')}</strong><small>{ar ? 'فساتين' : 'DRESSES'}</small></div>
            {circleProducts.map(({ product, index, angle }) => <button key={product.slug} className={`saffa-clock-item ${index === activeIndex ? 'active' : ''}`} style={{ '--angle': `${angle}deg` } as React.CSSProperties} onClick={() => enterProduct(index)} aria-label={`${text.open} ${product.name}`}><span><img src={product.image} alt="" /></span><small>{String(index + 1).padStart(2, '0')}</small></button>)}
          </div>
        ...
        <div className={`saffa-product-zoom ${view === 'product' ? 'is-visible' : 'is-hidden'}`}>
          <div className="saffa-product-track" style={{ transform: `translate3d(0, -${activeIndex * 100}%, 0)` }}>
            {products.map((product, index) => <article className="saffa-product-slide" key={product.slug} aria-hidden={index !== activeIndex}><button className="saffa-product-visual" onClick={() => setDetailOpen(true)} aria-label={`${text.open} ${product.name}`}><img src={product.image} alt={product.name} draggable={false} /></button></article>)}
          </div>
          ...
          <div className="saffa-product-meta"><span>{activeProduct.color}</span><h1>{activeProduct.name}</h1><strong>EGP {PRICE.toFixed(2)}</strong><button onClick={() => setDetailOpen(true)}>{text.open}</button></div>
          ...
          <button className="saffa-return-circle" type="button" onClick={() => setView('circle')}>{text.circle}</button>
        </div>
      </section>
