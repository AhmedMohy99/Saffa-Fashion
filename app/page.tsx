...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      setRotation(value => value + event.deltaY * 0.08);
      if (Math.abs(event.deltaY) > 8) setActiveIndex(value => event.deltaY > 0 ? (value + 1) % products.length : (value - 1 + products.length) % products.length);
    };
  ...
  function changeQuantity(slug: string, size: Size, delta: number) {
    setCart(current => current.map(item => item.slug === slug && item.size === size ? { ...item, quantity: item.quantity + delta } : item).filter(item => item.quantity > 0));
  }
    ...
    <main className="saffa-store" id="top">
      <header className="store-header blueprint-header">
        ...
        <nav className={`blueprint-nav ${mobileMenuOpen ? 'mobile-open' : ''}`} aria-label="Main navigation">
          ...
          <button className={viewMode === 'grid' ? 'nav-view active' : 'nav-view'} onClick={() => setViewMode('grid')}>Grid</button>
          <button className={viewMode === 'circle' ? 'nav-view active' : 'nav-view'} onClick={() => setViewMode('circle')}>Circle</button>
          <button className="cart-link" onClick={() => setCartOpen(true)}>Cart {cartCount}</button>
        </nav>
        <button className="mobile-menu-toggle" onClick={() => setMobileMenuOpen(value => !value)} aria-label="Menu"><span/><span/><span/></button>
      </header>
      ...
      <section id="collection" className={`blueprint-showcase ${viewMode === 'grid' ? 'is-grid' : 'is-circle'} ${quickView ? 'is-zoomed' : ''}`}>
        ...
        <div className="showcase-stage" style={{ ['--wheel-rotation' as string]: `${rotation}deg` }}>
        ...
        <div className="showcase-footer"><span>FAQ</span><span>Terms</span><span>Privacy</span><span>Scroll to rotate · Click a piece to explore</span></div>
      </section>
      ...
      {detailOpen && <div className="detail-backdrop" onClick={() => setDetailOpen(false)}><aside className="detail-panel" onClick={event => event.stopPropagation()}><div className="detail-header"><span className="quick-label">SAFFA FASHION · DRESSES</span><button onClick={() => setDetailOpen(false)}>Close ×</button></div><div className="detail-image"><img src={activeProduct.image} alt={activeProduct.name}/></div><span className="eyebrow">{activeProduct.color}</span><h2>{activeProduct.name}</h2><strong className="detail-price">{PRICE_LABEL}</strong><p>{activeProduct.description}</p><p className="arabic-copy">{activeProduct.arDescription}</p><div className="detail-swatches">{products.slice(Math.max(0, activeIndex - 1), activeIndex + 2).map((item, index) => <button key={item.slug} onClick={() => setActiveIndex(Math.max(0, activeIndex - 1) + index)}><img src={item.image} alt=""/></button>)}</div><div className="size-picker"><div><span>Select size</span><small>Weight guide</small></div>{(['L','XL'] as Size[]).map(size => <button key={size} className={selectedSize === size ? 'selected' : ''} onClick={() => setSelectedSize(size)}><b>{size}</b><small>{SIZE_INFO[size]}</small></button>)}</div><button className="detail-cart" onClick={addToCart}>Add to Cart <span>{PRICE_LABEL}</span></button><p className="detail-helper">Swipe, scroll, or use ← → to browse.</p><div className="detail-arrows"><button onClick={() => setActiveIndex(value => (value - 1 + products.length) % products.length)}>← Previous</button><button onClick={() => setActiveIndex(value => (value + 1) % products.length)}>Next →</button></div></aside></div>}
      ...
      <footer className="blueprint-footer"><span>© Saffa Fashion</span><nav><a href="#">FAQ</a><a href="#">Terms</a><a href="#">Privacy</a><a href={INSTAGRAM_URL} target="_blank" rel="noreferrer">Instagram</a><a href={TIKTOK_URL} target="_blank" rel="noreferrer">TikTok</a></nav></footer>
    </main>
  );
}
