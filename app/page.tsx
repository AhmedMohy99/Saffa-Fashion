...
export default function Home() {
  ...
  function openProduct(index: number) { setActiveIndex(index); setDetailOpen(true); }
  ...
  function handleTouchStart(event: React.TouchEvent<HTMLDivElement>) { setDragStart(event.touches[0].clientY); }
  ...
  return <main className="saffa-store saffa-swipe-viewer" id="top">
    <header className="store-header blueprint-header">
      ...
      <nav className="blueprint-nav" aria-label="Main navigation">
        ...
        <span className="nav-view active">Swipe</span>
    ...
    <section id="collection" className="saffa-swipe-viewer">
      ...
      <div className="swipe-stage" onWheel={handleWheel} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        ...
        <button className="swipe-dial" style={{ ['--progress' as string]: progress }} onClick={() => moveProduct(1)} aria-label="Next product"><span>{String(activeIndex + 1).padStart(2, '0')}</span></button>
        <div className="swipe-hint">Swipe to swap</div>
      </div>
      <div className="swipe-progress" style={{ width: `${progress * 100}%` }} />
      <div className="swipe-info">
        <div className="swipe-info-copy"><span className="swipe-eyebrow">SAFFA FASHION · {String(activeIndex + 1).padStart(2, '0')} / {String(products.length).padStart(2, '0')}</span><h2>{activeProduct.name}</h2><strong className="swipe-price">{PRICE_LABEL}</strong></div>
        <button className="swipe-open" onClick={() => setDetailOpen(true)} aria-label={`Open ${activeProduct.name}`}><img src={activeProduct.image} alt=""/><span>Open</span></button>
      </div>
    </section>

    {detailOpen && <div className="swipe-detail-backdrop" onClick={() => setDetailOpen(false)}><aside className="swipe-detail" onClick={event => event.stopPropagation()}>
      ...
      <span className="swipe-eyebrow">{activeProduct.color}</span><h2>{activeProduct.name}</h2><strong className="swipe-detail-price">{PRICE_LABEL}</strong>
      <p>{activeProduct.description}</p><p>{activeProduct.arDescription}</p>
    ...
    {cartOpen && <div className="swipe-detail-backdrop" onClick={() => setCartOpen(false)}><aside className="swipe-detail" onClick={event => event.stopPropagation()}>
