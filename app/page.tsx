...
const SIZE_GUIDE: Record<Size, { chest: number; body: number; sleeve: number }> = {
  XS: { chest: 48, body: 135, sleeve: 60 },
  S: { chest: 50, body: 136, sleeve: 61 },
  M: { chest: 52, body: 137, sleeve: 62 },
  L: { chest: 54, body: 138, sleeve: 63 },
  XL: { chest: 56, body: 139, sleeve: 64 },
  '2XL': { chest: 58, body: 140, sleeve: 65 },
};
  ...
  return (
    <main className="saffa-store" id="top">
      ...
      {detailOpen && <div className="detail-backdrop" onClick={() => setDetailOpen(false)}><aside className="detail-panel" onClick={event => event.stopPropagation()}>
        ...
        <span className="eyebrow">{activeProduct.color}</span><h2>{activeProduct.name}</h2><strong className="detail-price">{PRICE_LABEL}</strong>
        <p>{activeProduct.description}</p><p className="arabic-copy">{activeProduct.arDescription}</p>
        <p className="shipping-note">Delivery and shipping details are confirmed with your Saffa order.</p>
        ...
        <section className="sizing-section"><div className="sizing-heading"><div><span className="quick-label">SIZE GUIDE</span><h3>Select your size</h3></div><span>All measurements in cm</span></div>
          <div className="size-selector">{SIZES.map(size => <button key={size} className={selectedSize === size ? 'selected' : ''} onClick={() => setSelectedSize(size)}>{size}</button>)}</div>
          <div className="sizing-table-wrap"><table className="sizing-table"><thead><tr><th>Measurement</th>{SIZES.map(size => <th key={size}>{size}</th>)}</tr></thead><tbody><tr><td>Chest Width</td>{SIZES.map(size => <td key={size}>{SIZE_GUIDE[size].chest}</td>)}</tr><tr><td>Body Length</td>{SIZES.map(size => <td key={size}>{SIZE_GUIDE[size].body}</td>)}</tr><tr><td>Sleeve Length</td>{SIZES.map(size => <td key={size}>{SIZE_GUIDE[size].sleeve}</td>)}</tr></tbody></table></div>
          <p className="size-footnote">* All measurements are in centimeters. For the best fit, compare these garment measurements with a similar dress you already own.</p>
        </section>
