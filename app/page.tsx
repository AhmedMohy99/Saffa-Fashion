...
export default function Home() {
  ...
  function addToCart(product: Product = activeProduct) { setCart(current => { const found = current.find(item => item.slug === product.slug); if (found) return current.map(item => item.slug === product.slug ? { ...item, quantity: item.quantity + 1, price: product.price, color: product.color, arColor: product.arColor, name: product.name, arName: product.arName } : item); return [...current, { slug: product.slug, name: product.name, arName: product.arName, image: product.image, price: product.price, quantity: 1, color: product.color, arColor: product.arColor }]; }); setDetailOpen(false); setCartOpen(true); }
  ...
  function buyNow(product: Product = activeProduct) { const next = [{ slug: product.slug, name: product.name, arName: product.arName, image: product.image, price: product.price, quantity: 1, color: product.color, arColor: product.arColor }]; localStorage.setItem('saffa-cart', JSON.stringify(next)); window.location.href='/checkout'; }
  ...
  const localizedName = (product: Product) => ar ? product.arName : product.name;
  const localizedColor = (product: Product) => ar ? product.arColor : product.color;
